...
class AuthOIDCView(AuthOIDView):
    ...
    def logout(self):
        ...
        return redirect(
            oidc.client_secrets.get('issuer') + '/protocol/openid-connect/logout?client_id='+ oidc.client_secrets.get('client_id'))
    ...
    def _attach_roles_for(self, user, default_roles: list[str] = None):
        ...
        lookfor_roles = ["admin", "alpha", "gamma", "public"] # roles supportés par notre adapter
        ...
        for role in roles_to_apply:
            ...
            if found is None:
                logger.warning(f"Role {role} not found in superset. It will be overlooked!")
                continue
            roles.append(found)
        
        user.roles = roles
        
    def _intersection_insensible_casse(self, lst1: list[str], lst2: list[str]) -> list[str]:
        """Intersection entre deux list de str (insensible à la casse)"""
        upper2 = [x.upper() for x in lst2]
        intersection = [
            v for v in lst1
            if v.upper() in upper2
        ]
        return intersection
